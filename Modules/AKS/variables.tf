...
variable "net_profile_outbound_type" {
  ...
  # using regex
  validation {
    condition     = can(regex("^(loadBalancer|userDefinedRouting)$", var.net_profile_outbound_type))
    error_message = "Invalid input, options: \"loadBalancer\", \"userDefinedRouting\"."
  }
}
...
variable "network_plugin" {
  ...
  # using regex
  validation {
    condition     = can(regex("^(kubenet|azure)$", var.network_plugin))
    error_message = "Invalid input, options: \"kubenet\", \"azure\"."
  }
}
...
variable "sku_tier" {
  ...
  validation {
    ...
    error_message = "Invalid input, options: \"Free\", \"Paid\"."
  }
}
